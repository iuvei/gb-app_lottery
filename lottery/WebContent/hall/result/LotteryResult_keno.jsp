    ...
    <ul>
        ...
        <c:forEach var="item" items="${command.result}" varStatus="vs">
            <li style="margin-bottom: 10px;">
                ...
                <span class="sp5"><c:if test="${numSum == 810}">和</c:if><c:if test="${numSum%2 == 0 && numSum != 810}">双</c:if><c:if test="${numSum%2 != 0}">单</c:if></span>
                <span class="sp5"><c:if test="${numSum>810}">大</c:if><c:if test="${numSum < 810}">小</c:if><c:if test="${numSum == 810}">和</c:if></span>
